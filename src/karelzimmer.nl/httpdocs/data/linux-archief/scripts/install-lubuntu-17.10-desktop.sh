###############################################################################
# Bestand:  install-lubuntu-17.10-desktop.sh
# Doel:     Installatiebestand voor Lubuntu 17.10 desktop.
# Gebruik:  1. In het terminalvenster met script install (aanbevolen):
#              install ~/bin/install-lubuntu-17.10-desktop.sh
#           2. Direct in het terminalvenster:
#              bash ~/bin/install-lubuntu-17.10-desktop.sh
...
#              install -s ~/bin/install-lubuntu-17.10-desktop.sh
...
## ===================================
# Pakketbron toevoegen, [arch=amd64] omdat anders fout 'missende i386-pakketten' op Lubuntu 64-bit.
[[ $(dpkg --print-architecture) != 'amd64' ]] || echo 'deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main' | sudo tee /etc/apt/sources.list.d/google-chrome.list

# Sleutel toevoegen.
[[ $(dpkg --print-architecture) != 'amd64' ]] || wget --output-document=- 'https://dl-ssl.google.com/linux/linux_signing_key.pub' | sudo apt-key add -
...
## ==========================================
# Pakketbron toevoegen.
[[ $(dpkg --print-architecture) != 'amd64' ]] || echo 'deb https://repo.skype.com/deb stable main' | sudo tee /etc/apt/sources.list.d/skype-stable.list

# Sleutel toevoegen.
[[ $(dpkg --print-architecture) != 'amd64' ]] || wget --output-document=- 'https://repo.skype.com/data/SKYPE-GPG-KEY' | sudo apt-key add -

# Pakketbronnen verversen.
[[ $(dpkg --print-architecture) != 'amd64' ]] || sudo apt-get update

# Pakket installeren.
[[ $(dpkg --print-architecture) != 'amd64' ]] || sudo apt-get install --yes skypeforlinux
